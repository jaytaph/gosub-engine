...
fn main() -> Result<()> {
    let matches = clap::Command::new("Gosub CSS3 parser")
        ...
        .arg(
            clap::Arg::new("ast")
                .help("Display abstract syntax tree (AST)")
                .long("ast")
                .action(clap::ArgAction::SetTrue),
        )
        .arg(
    ...
    let display_ast = matches.get_flag("ast");
    ...
    let now = Instant::now();
    let result = Css3::parse_stylesheet(css.as_str(), config);
    ...
    println!("\n------- Log messages -------");
    for log in tree.logs.iter() {
        println!("{}", log);
    }
    println!("----------------------------\n");

    if match_values {
        // if let Ok(css_tree) = convert_ast_to_stylesheet(&tree, CssOrigin::UserAgent, "test.css".into()) {
        //     let matcher = MatchWalker::new(&css_tree);
        //     matcher.walk();
        // } else {
        //     println!("Failed to convert AST to stylesheet");
        // }
    }
...
fn print_ast(css: &str) {
    let tree = Css3::parse_to_ast(css.as_str(), Default::default())?;
    println!("{:#?}", tree);
}
