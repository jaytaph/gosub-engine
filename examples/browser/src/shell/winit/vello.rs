//! winit shell for the Vello (GPU) rasterizer.
//!
...
use crate::common::rt;
use crate::common::wgpu_ctx::TextureMapContextProvider;
...
struct RuntimeState {
    ...
    context: Arc<TextureMapContextProvider>,
    gpu: GpuState,
...
impl ApplicationHandler<()> for BrowserApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let adapter = match rt::rt().block_on(self.instance.request_adapter(&wgpu::RequestAdapterOptions {
        ...
        let (device, queue) = match rt::rt().block_on(adapter.request_device(&wgpu::DeviceDescriptor::default())) {
            Ok(dq) => dq,
            Err(e) => {
        ...
        let context = Arc::new(TextureMapContextProvider::new(device.clone(), queue.clone()));
        ...
        let setup = setup_engine(
            Arc::new(backend),
            self.compositor.clone(),
            &SetupOptions {
                zone_uuid: uuid::uuid!("f1234567-abcd-4000-8000-000000000007"),
                tab_title: "Gosub",
                local_store_path: ":memory:",
                initial_viewport: Some(Viewport::new(0, 0, size.width, size.height)),
            },
        );
