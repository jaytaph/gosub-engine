//! egui shell for the Vello (GPU) rasterizer.
//!
//! No GTK dependency — pure egui + wgpu. The wgpu device/queue are borrowed
...
impl BrowserApp {
    fn new(cc: &CreationContext, initial_url: String) -> Option<Self> {
        ...
        let context = Arc::new(TextureMapContextProvider::new(
            Arc::new(ws.device.clone()),
            Arc::new(ws.queue.clone()),
        ));
        ...
        let setup = setup_engine(
            ...
            compositor.clone(),
            &SetupOptions {
                zone_uuid: uuid::uuid!("f1234567-abcd-4000-8000-000000000008"),
                tab_title: "Gosub",
                ...
                initial_viewport: Some(Viewport::new(0, 0, 1024, 768)),
            },
        );
...
pub fn run(initial_url: String) -> Result<(), eframe::Error> {
