//! GTK4 shell for CPU rasterizers (Cairo, Skia).
//!
//! The engine renders pages through the 7-stage pipeline (rendertree → layout →
//! layering → tiling → painting → rasterize → composite); this shell composites
//! the resulting TileCache frames into a GTK4 `DrawingArea`. The two CPU backends
//! share everything except construction and DPR handling, expressed through
//! [`Gtk4CpuOptions`].

use crate::common::engine::{setup_engine, SetupOptions};
use crate::common::{nav, rt};
use gosub_engine::events::{EngineEvent, NavigationEvent, TabCommand};
use gosub_render_pipeline::render::backend::{CachedTile, ExternalHandle, RenderBackend};
...
/// Per-binary knobs for the GTK4 CPU shell.
pub struct Gtk4CpuOptions {
    pub app_id: &'static str,
    pub window_title: &'static str,
    pub default_url: &'static str,
    pub setup: SetupOptions,
    pub make_backend: fn() -> Arc<dyn RenderBackend + Send + Sync>,
    /// Where to publish the window's scale factor on resize. Cairo rasterizes at
    /// physical pixels and reads its DEVICE_PIXEL_RATIO atomic; Skia rasterizes
    /// at CSS pixels and passes `None`.
...
pub fn run(opts: Gtk4CpuOptions) {
    let initial_url = nav::initial_url_from_args(opts.default_url);

    let app = Application::builder().application_id(opts.app_id).build();

    let window_title = opts.window_title;
    let make_backend = opts.make_backend;
    let dpr_sink = opts.dpr_sink;
    let setup_opts = opts.setup;

    app.connect_activate(move |app| {
        let _rt_guard = rt::rt().enter();
        ...
        // (No-op for Skia builds — it only does work under backend_cairo_pango.)
        ...
        let setup = setup_engine(make_backend(), compositor.clone(), &setup_opts);
        let tab_id = setup.tab_id;
        let event_rx = setup.event_rx;

        // Wrap the tab in Rc<RefCell<>> so closures can share it
        let tab = Rc::new(RefCell::new(setup.tab));
        ...
        // Navigate after the window is shown so the viewport is already set.
        {
            ...
            glib::idle_add_local_once(move || {
                let s = nav::normalize_url(&url_str);
