...
trait ConfigurationModule {
    type Global:GlobalConfuration;
    type Html5: Html5ParserConfuration;
    type Css3: Html5ParserConfuration;
}

trait GlobalConfuration {
    type NavigationId;
    type RequestId;
    type TabId;
    type DecisionToken;
...
struct ComplexNavigationId {
    part1: u64,
    part2: u64,
    part3: String,
}

impl Display for ComplexNavigationId {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}-{}-{}", self.part1, self.part2, self.part3)
    }
}

type NavigationId = u64;
type NavigationIdUuid = uuid::Uuid;

struct GlobalConfig;

impl GlobalConfiguration for GlobalConfig {
    type NavigationId = NavigationIdUuid;
}

trait NavigationID: ToString + Debug {
}

struct ConfigModule;

impl ConfigurationModule for ConfigModule {
    type Global = GlobalConfig;
    type Html = Html5Config;
    ...
}


GosubEngine<Config>::new()


/// Commands that can be sent to a specific tab
#[derive(Clone, Debug, PartialEq)]
pub enum TabCommand<C: ConfigurationModule> {
    // ****************************************
    ...
    SubmitDecision {
        nav_id: t!(Global::NavigationId),
        decision_token: t!(Global::DecisionToken),
...
#[derive(Debug, Clone)]
pub enum NavigationEvent<C: ModuleConfiguration> {
    /// Navigation has been started
    Started { nav_id: t!(Global::NavigationId), url: Url },
